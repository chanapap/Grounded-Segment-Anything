...
grounding_dino_model = load_model(GROUNDING_DINO_CONFIG_PATH,
...
def filter_small_masks(masks, boxes, scores, image_shape, min_area_ratio=0.1):
    """
    ...
    filtered_scores = np.array(scores)[keep]

    return filtered_masks, filtered_boxes, filtered_scores

def filter_noisy_masks(masks, boxes, scores, max_components=30, min_component_area=5):
    """
    ...
    return clean_masks, np.array(clean_boxes),  np.array(clean_scores)


def remove_masks_that_contain_others(masks, boxes, scores, threshold=0.9):
    """
...
def get_curtain_bboxes_points(SOURCE_IMAGE_PATH):

    BOX_THRESHOLD = 0.55
    TEXT_THRESHOLD = 0.55
    NMS_THRESHOLD = 0.8

    TEXT_PROMPT = "curtains"

    print("Getting curtain masks from ", SOURCE_IMAGE_PATH)
    ...
    if os.path.exists(SOURCE_IMAGE_PATH):
        ...
        final_masks, final_boxes, filtered_scores = remove_masks_that_contain_others(
        ...
        detections_masks = final_masks
        detections_box = final_boxes
        ...
        print("CURTAIN SCORES", filtered_scores)

    return detections_box, selected_points
...
def get_rug_masks(SOURCE_IMAGE_PATH):
    ...
    #     for _, _, confidence, class_id, _, _ 
    #     in detections]
    all_rug_bbox = detections_box
    all_rug_masks = detections_mask
    
    # print("RUG Labels", all_rug_labels)
    # print("RUG BBOX", all_rug_bbox)
    # print("RUG Masks", all_rug_masks)

    return all_rug_bbox, all_rug_masks
...
def get_floor_masks(SOURCE_IMAGE_PATH):
    ...
    #     for _, _, confidence, class_id, _, _ 
    #     in detections]
    all_floor_bbox = detections_box
    all_floor_masks = detections_mask
    
    # print("FLOOR Labels", all_floor_labels)
    # print("FLOOR BBOX",   all_floor_bbox)
    # print("FLOOR Mask",   all_floor_masks)

    return all_floor_bbox, all_floor_masks


def filter_floor_masks(all_floor_bbox, floor_masks):
    """
    ...
    if len(floor_masks) <= 1:
        return all_floor_bbox, floor_masks
    ...
    return filtered_floor_bbox, filtered_floor_masks
...
def get_10th_percentile_true_point(mask):
    ...
    return [int(coords[idx][1]), int(coords[idx][0])]  # (row, col)

def filter_rugs_with_floor_update_and_revert(
    floor_bboxes, floor_masks,
    rug_bboxes, rug_masks,
    ...
    for i, (rug_mask, rug_bbox) in enumerate(zip(rug_masks, rug_bboxes)):
        ...
        if rug_valid:
            ...
            # get point from valid rug mask to use as negative point for floor
            mask_uint8 = rug_mask.astype(np.uint8)
            # Compute center from moments
            moments = cv2.moments(mask_uint8)
            if moments["m00"] != 0:
                original_cx = int(moments["m10"] / moments["m00"])
                ...
                ys, xs = np.where(rug_mask)
                original_cx = int(np.mean(xs))
                original_cy = int(np.mean(ys))
            cx, cy = original_cx, original_cy
            if not rug_mask[cy, cx]:
                ys, xs = np.where(rug_mask)
                distances = np.sqrt((xs - cx)**2 + (ys - cy)**2)
                # Get index of 25th percentile closest point
                percentile_index = int(len(distances) * 0.2)
...
def get_floor_bboxes_points_with_rug(SOURCE_IMAGE_PATH):
    ...
    all_rug_bbox, all_rug_masks = get_rug_masks(SOURCE_IMAGE_PATH)
    all_floor_bbox, all_floor_masks = get_floor_masks(SOURCE_IMAGE_PATH)

    # image = cv2.imread(SOURCE_IMAGE_PATH)

    filtered_floor_bbox, filtered_floor_masks = filter_floor_masks(all_floor_bbox, all_floor_masks)
    ...
    return np.array(filtered_floor_bbox), selected_floor_points, valid_rug_points
...
def show_point(point, ax, marker_size=100):
    ax.scatter(point[0], point[1], color='blue', marker='.', s=marker_size, edgecolor='white', linewidth=1.25)    
